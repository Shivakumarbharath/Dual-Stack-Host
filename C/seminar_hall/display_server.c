...
void Char_to_Binary(unsigned char s)
{
	int binary[8];
	for(int n = 0; n < 8; n++)
	binary[7-n] = (s >> n) & 1;
...
date_status check_status(month *mon)
{
	...
	for(int i=0;i<31;i++)
	{printf("\n\n Date - %d-%02d-%04d \t slots ",i+1,status.month,status.year);Char_to_Binary(status.status[i]);}
	return status;
}
...
void Book(int cli_sock,exchange resp, month *h1)
{
int n;
date_status stat=check_status(h1);
	if(h1->date1[resp.event_day].slots[resp.slot_no].booking_status==0)
		...
		strcpy(h1->date1[resp.event_day].slots[resp.slot_no].booking_id,booking_id(resp,h1_cur_month.mon,h1_cur_month.year));
		h1->date1[resp.event_day].day_status|=Slot_to_Binary(resp.slot_no+1);
		//printf("Going into the if statment");
		n=send(cli_sock,(void *)&h1->date1[resp.event_day].slots[resp.slot_no],sizeof(h1->date1[resp.event_day].slots[resp.slot_no]),0);
...
void Cancel_date(int cli_sock,exchange resp, month *h1)
{
int n;
date_status stat=check_status(h1);
...
void Cancel_id(int cli_sock,exchange resp, month *h1)
{
