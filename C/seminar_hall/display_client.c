...
//printf("Sent time= %s\n\n",Time());
  ...
  slot con;
  printf
    ("\n\n^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n\tWELCOME TO SEMINAR HALL BOOKING APP\n^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n\tChoose the following\n\n\n\t1.Booking\n\n\n\t2.Cancelling by Date\n\n\n\t3.Cancelling by Booking ID\n\n\n\t4.Display Booked Dates in a Month\n\n\n\t5.Exit\n\n");
  printf
    ("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^\n\nChoice : ");
  scan (&req.type);
  if (req.type == 5)
...
//      display_status(stat);
	...
	req.hall -= 1;
	req.
	  hall ? printf ("\nYou had chosed New seminar Hall\n") :
	  printf ("\nYou Had chosed Old seminar hall\n");
	printf ("\n\n\n\t1.Current Month\n\n\n\t2.Next Month\n\n\tChoice? :");
	scan (&req.month_type);
	req.month_type -= 1;
	req.
    ...
    case 2:
      {
	printf
	  ("\n\n\tYou have Chosed Cancellation by Booking ID..\n\n\n\tEnter Booking ID seperated by '-' :");
