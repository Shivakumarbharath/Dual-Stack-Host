...
{	
	...
	if(resp.hall==0 && resp.month_type==0)
		{
		date_status stat=check_status(h1_cur_month);
		...
		{
		date_status stat=check_status(h1_nxt_month);
	        ...
	        {
	        date_status stat=check_status(h2_cur_month);
        	...
        	{
        	date_status stat=check_status(h2_nxt_month);
...
case 2:
	...
	{
		//
			if(h1_cur_month.date1[day].slots[slot].booking_status==1)
          	        { 
          	        Display_slot(h1_cur_month.date1[day].slots[slot]);
          	        //make booking id =0
          	        strcpy(h1_cur_month.date1[day].slots[slot].booking_id,"");
          	        //change the status
                         h1_cur_month.date1[day].slots[slot].booking_status=0;
			Printf("Booking Cancelled!!");

                         //remove the name 
                         strcpy(h1_cur_month.date1[day].slots[slot].name,"");
                         strcpy(h1_cur_month.date1[day].slots[slot].contact,"");
                         char cancel_report[]={"Booking Cancelled!!"};
                         n=send(cli_sock,&cancel_report,sizeof(cancel_report),0);
                         if(n<0){printf("Error sending Cancellation Report\nTry again");close(cli_sock);}
	...
	{
		//
		if(h1_nxt_month.date1[day].slots[slot].booking_status==1)
          	        { 
          	        Display_slot(h1_nxt_month.date1[day].slots[slot]);
          	        //make booking id =0
          	        strcpy(h1_nxt_month.date1[day].slots[slot].booking_id,"");
          	        //change the status
                         h1_nxt_month.date1[day].slots[slot].booking_status=0;
			Printf("Booking Cancelled!!");

                         //remove the name 
                         strcpy(h1_nxt_month.date1[day].slots[slot].name,"");
                         strcpy(h1_nxt_month.date1[day].slots[slot].contact,"");
                         char cancel_report[]={"Booking Cancelled!!"};
                         n=send(cli_sock,&cancel_report,sizeof(cancel_report),0);
                         if(n<0){printf("Error sending Cancellation Report\nTry again");close(cli_sock);}
	...
	else if(h1_cur_month.mon==month && hall==1)
	{
		...
		if(h2_cur_month.date1[day].slots[slot].booking_status==1)
          	        { 
          	        Display_slot(h2_cur_month.date1[day].slots[slot]);
          	        //make booking id =0
          	        strcpy(h2_cur_month.date1[day].slots[slot].booking_id,"");
          	        //change the status
                         h2_cur_month.date1[day].slots[slot].booking_status=0;
			Printf("Booking Cancelled!!");

                         //remove the name 
                         strcpy(h2_cur_month.date1[day].slots[slot].name,"");
                         strcpy(h2_cur_month.date1[day].slots[slot].contact,"");
                         char cancel_report[]={"Booking Cancelled!!"};
                         n=send(cli_sock,&cancel_report,sizeof(cancel_report),0);
                         if(n<0){printf("Error sending Cancellation Report\nTry again");close(cli_sock);}
	...
	{
		//
		if(h2_nxt_month.date1[day].slots[slot].booking_status==1)
          	        { 
          	        Display_slot(h2_nxt_month.date1[day].slots[slot]);
          	        //make booking id =0
          	        strcpy(h2_nxt_month.date1[day].slots[slot].booking_id,"");
          	        //change the status
                         h2_nxt_month.date1[day].slots[slot].booking_status=0;
			Printf("Booking Cancelled!!");

                         //remove the name 
                         strcpy(h2_nxt_month.date1[day].slots[slot].name,"");
                         strcpy(h2_nxt_month.date1[day].slots[slot].contact,"");
                         char cancel_report[]={"Booking Cancelled!!"};
                         n=send(cli_sock,&cancel_report,sizeof(cancel_report),0);
                         if(n<0){printf("Error sending Cancellation Report\nTry again");close(cli_sock);}
...
break;}// case
default:Printf("Service not availible");
}//switch
